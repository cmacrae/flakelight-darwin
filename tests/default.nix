...
let
  ...
  inherit (nixpkgs) lib;
in {
  call-flakelight-darwin =
    test (flakelight-darwin ./empty { outputs.test = true; }) (f: f.test);
  ...
  darwinModule = test (flakelight-darwin ./empty { darwinModule = _: { }; })
    (f: f ? darwinModules.default);

  darwinModules =
    test (flakelight-darwin ./empty { darwinModules.test = _: { }; })
